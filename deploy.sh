...
FB_API_KEY="AIzaSyDH8Plz5dS7VaY6EX4a1OacBcnAgx8vnXc"
...
deploy_agent() {
  ...
  (cd "$PROJECT_DIR/recipe-agent" && gcloud run deploy "$AGENT_SERVICE" \
    ...
    --region "$REGION" \
    --allow-unauthenticated \
    --set-env-vars "USE_VERTEX_AI=false,GEMINI_API_KEY=${GEMINI_API_KEY},FIREBASE_STORAGE_BUCKET=${FB_STORAGE_BUCKET},YOUTUBE_TRANSCRIPT_DEV_API_KEY=${YT_TRANSCRIPT_KEY}")
